...
# Default to the current integrated repository (xiumuzidiao0/aimili-vpngate)
DEFAULT_USER="xiumuzidiao0"
...
if [ "$PKG_MGR" = "apt-get" ]; then
    ...
    apt-get install -y qrencode || echo -e "${YELLOW}  -> 警告: qrencode 安装失败，WebUI 二维码暂不可用，其余功能不受影响。${PLAIN}"
    ...
    apk add libqrencode-tools || echo -e "${YELLOW}  -> 警告: libqrencode-tools 安装失败，WebUI 二维码暂不可用，其余功能不受影响。${PLAIN}"
    ...
    $PKG_MGR install -y qrencode || echo -e "${YELLOW}  -> 警告: qrencode 安装失败，WebUI 二维码暂不可用，其余功能不受影响。${PLAIN}"
fi
